...
/* ---- links in body copy that no rule ever coloured, so they fell back to the
   browser's default blue. :where() holds specificity at zero, so any rule that
   does style a link still wins — including the gold ones on dark panels. ---- */
:where(p,li,td,dd,figcaption) :where(a:not([class])){color:#6E56A4;
 text-decoration:underline;text-underline-offset:2px;text-decoration-thickness:1px}
:where(p,li,td,dd,figcaption) :where(a:not([class]):hover){color:#8A6410}
